         ...
         return Provider.request(target) { (result) in
             ...
             case let .success(response):
                 do {
                    ...
                    let error_code: Int = jsonData["error_code"].rawValue as! Int
//                    let resultcode: String = jsonData["resultcode"].rawValue as! String
                    if error_code == 0 {
                        completion(jsonData)
                    }else{
                        let reason: String = jsonData["reason"].rawValue as! String
                        failed?(reason)
                    }
                     //print(jsonData)
